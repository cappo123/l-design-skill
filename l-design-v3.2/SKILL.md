...
| `figma-patterns.md` | Vibma API call templates, variable lookup, Figma troubleshooting | Figma mode only |
| `frontend.md` | CSS classes, React examples, noise texture, icons | Frontend mode only |
| `motion-tokens.md` | Motion principles, timing, easing tokens, reduced-motion | When adding motion/interaction — read this first |
| `motion-patterns.md` | Component animation implementations (React + CSS) | When implementing specific motion patterns |
| `golden-sample.md` | Complete dashboard output example (HTML + Figma structure) | When verifying output quality |
